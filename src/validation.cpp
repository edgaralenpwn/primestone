...
//1 year = 90second * 350400
    ...
    if (nPrevHeight < 1)
	dSubsidyBase = 500000;
	else if (nPrevHeight < 14400)			//during 2 weeks
	dSubsidyBase = 50;	
    else if (nPrevHeight < 364800)			//after 1 year  
	dSubsidyBase = 40;				
    else if (nPrevHeight < 715200)			//after 1 year
	dSubsidyBase = 30;				
    else if (nPrevHeight < 1065600)
	dSubsidyBase = 20;
    else if (nPrevHeight < 1416000)
	dSubsidyBase = 10;
    else if (nPrevHeight < 1766400)
	dSubsidyBase = 5;
    else if (nPrevHeight < 2116800)
	dSubsidyBase = 1;
    else 
	dSubsidyBase = 0.5;
...
/*
	...
	dSubsidyBase = 36;
    else if (nPrevHeight < 364800)
	dSubsidyBase = 18;
    else if (nPrevHeight < 715200)
	dSubsidyBase = 15.3;
    else if (nPrevHeight < 1065600)
	dSubsidyBase = 10.71;
    else if (nPrevHeight < 1416000)
...
{
    CAmount ret = blockValue * 3 / 10; // start at 30%
...
*/
	if(nHeight > 364800) ret = blockValue * 5 / 10 ; //mainnet: 50% after 1 year
    if(nHeight > 1065600) ret = blockValue * 7 / 10 ; //mainnet: 70% after 2 year
    if(nHeight > 1766400) ret = blockValue * 9 / 10; //mainnet: 90% after 2 year
//    if(nHeight > 1416000) ret = blockValue; //mainnet: 45000000 - 100% after 1 year
...
bool IsInitialBlockDownload()
{
    ...
    if (fImporting || fReindex)
        return true;
//    printf("fImporting: %d, fReindex: %d..................................\n", fImporting, fReindex);
    LOCK(cs_main);
    const CChainParams& chainParams = Params();
    if (chainActive.Tip() == NULL)
        return true;
//	printf("chainActive.Tip() is not NULL.....................\n");
//	printf("chainActive.Tip()->nChainWork: %ld, nMininmumChainWork: %ld.................\n", chainActive.Tip()->nChainWork, UintToArith256(chainParams.GetConsensus().nMinimumChainWork));
    if (chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
        return true;

//	printf("chainActive.Tip()->GetBlockTime(): %ld, GetTime : %ld,  chainParams.MaxTipAge(): %ld.........\n", chainActive.Tip()->GetBlockTime(), GetTime(), chainParams.MaxTipAge());
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // PRIMESTONE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(PSC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(PSC): couldn't find masternode or superblock payments"),
...
bool CheckBlockHeaderFromPoS(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    // Check proof of work matches claimed amount
    if (fCheckPOW && !CheckProofOfStake(block.GetHash(), block.nBits, Params().GetConsensus()))
        return state.DoS(50, error("CheckBlockHeader(): proof of stake failed"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // PRIMESTONE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(PSC): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(PSC): spork is off, skipping transaction locking checks\n");
    }

...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "primestone (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all PrimeStone specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N PSC anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The PrimeStone Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n";
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    try {
        ...
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. PrimeStone is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (proxyArg != "" && proxyArg != "0") {
        CService resolved(LookupNumeric(proxyArg.c_str(), 31995));
    ...
    if (onionArg != "") {
        if (onionArg == "0") { // Handle -noonion/-onion=0
            ...
            CService resolved(LookupNumeric(onionArg.c_str(), 31995));
    ...
    // ********************************************************* Step 11c: update block tip in PrimeStone modules
    ...
    // ********************************************************* Step 11d: start primestone-ps-<smth> threads
